myplotellipses=function (model, keepvar = "all", axis = c(1, 2), means = TRUE, 
    level = 0.95, magnify = 2, cex = 0.5, pch = 20, pch.means = 15, 
    type = c("g", "p"), keepnames = TRUE, namescat = NULL, xlim = xlim, 
    ylim = ylim, lwd = 1, label = "all", autoLab = c("auto", 
        "yes", "no"), col='black', ...) 
{
    ...
    monpanel.ellipse <- function(x, y, col.line, lty, lwd, subscripts, 
        ...
        lpoints(cdg[1], cdg[2], pch = pchmeans[group.number], 
    ...
    if (class(model)[1] == "PCA") {
        if (all(names(model$call) != "quali.sup")) 
            ...
            if (is.character(keepvar)) {
                if (length(keepvar) == 1) {
                  ...
                  if (is.na(choix)) {
                    if (!any(keepvar == nomtot)) 
                      return(NULL)
                    else eliminer <- unique(c(eliminer, (1:nbevartot)[!(keepvar == 
                      nomtot)]))
                  }
                  else {
                    if (choix == 2) 
                ...
                else eliminer <- unique(c(eliminer, (1:nbevartot)[!(nomtot %in% 
                  keepvar)]))
            }
        ...
        if (is.character(keepvar)) {
            if (length(keepvar) == 1) {
                ...
                if (is.na(choix)) {
                  ...
                  if (choix == 1) {
                    if (all(!(names(model$call) %in% c("quali", 
                      "quali.sup")))) 
                      return(NULL)
                  }
                }
...
#                 else {
#                   if (choix == 2) 
#                     eliminer <- c(eliminer, which(model$call$nature.var == 
#                       "quali.sup"))
#                   if (choix == 3) 
#                     eliminer <- c(eliminer, which(model$call$nature.var == 
#                       "quali"))
...
#     else {
#         if (!is.null(eliminer)) 
#             donnees <- model$call$X[-model$call$ind.sup, -eliminer, 
#                 drop = FALSE]
#         else donnees <- model$call$X[-model$call$ind.sup, , drop = FALSE]
#     } 
    nbevar <- ncol(donnees)   # nombre de categories
    if (nbevar == 1) {
...
#         if (!is.null(model$call$ind.sup)) {
#           aux <- cbind.data.frame(model$call$X[-model$call$ind.sup,var], model$ind$coord[, axis])
#         }
#           else {
...
#           }
        if (class(model)[1] == "PCA") {
            ...
            plot.PCA(model, habillage = var, ellipse = coord.ell, 
                label = label, axes = axis, title = paste("Confidence ellipses around the categories of", 
                  colnames(model$call$X)[var]), autoLab = autoLab)
        }
#         if (class(model)[1] == "MCA") {
#             res.pca <- PCA(aux, quali.sup = 1, scale.unit = FALSE, 
#                 graph = FALSE)
#             res.pca$eig[axis, ] = model$eig[axis, ]
#             coord.ell <- coord.ellipse(aux, bary = means)
#             plot.PCA(res.pca, habillage = 1, ellipse = coord.ell, 
#                 cex = 0.8, label = label, axes = axis, title = paste("Confidence ellipses around the categories of", 
#                   colnames(model$call$X)[var]), autoLab = autoLab)
#         }
#         if (class(model)[1] == "MFA") {
#             res.pca <- PCA(aux, quali.sup = 1, scale.unit = FALSE, 
...
#         }
        ...
        xyplot(y ~ x | var, data = don, groups = modalite, panel = monpanel, 
            level = level, means = means, magnify = magnify, 
            cex = cex, pch = pch, pchmeans = pch.means, nommod = nommod, 
            type = type, xlab = paste("Dim ", axis[1], " (", 
                round(model$eig[axis[1], 2], 2), "%)", sep = ""), 
            ylab = paste("Dim ", axis[2], " (", round(model$eig[axis[2], 
                2], 2), "%)", sep = ""))
    }
}
