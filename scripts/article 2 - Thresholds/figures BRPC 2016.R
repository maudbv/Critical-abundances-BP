...
col <- rep("white",6)
col[ as.numeric( M$thresh[sp,threshold]):6] <- "black"  # identify the threshold

####### PLOT of lolium perenne alpha effect size: #######################
...
############## Barplot of frequencies for native richness only #############
...
############## Proportion of negative and postive ES for alien richness #############
...
par( mar=c(0,0,2,1), oma=c(6,4,1,1))
ylim = c(0,200)
xlim = c(-0.5, 5.5)

  sp <- "LOLPER"
  # identify significant classes in black
  cols <- c(NA, "black") [ ((gamma.trend.nat.permute.all$P.gamma[sp,] <=0.05)| (gamma.trend.nat.permute.all$P.gamma[sp,] >=0.975)) +1]
  cols[1]=NA
  ...
  # plot background
    plot(x,y,ylim=ylim, xlim = xlim, las = 1,type= "n", xaxt = "n", yaxt="n", ann=F,
    pch = 21, col ="black",bg = cols)

    axis(1,at = 1:5, label = rep("",5), tcl= 0.1,mgp=c(1,0.5,0),las=1)
    text(y=-15, x = 0:5, labels= abclasses[1:6],  cex=1, srt=45, adj=1, xpd = NA)
    ...
    polygon(na.omit(cix), na.omit(cinull.all), col="#FF875B", border="#FF875B")
    text(x, 200,label =n, col = "#FF875B" )
  ...
  # Add species name
#   mtext(3, text=paste(species[sp, "Genus"]," ", species[sp, "Species"], sep="") ,
#         font = 3, outer= F,adj=0.9, cex=0.9, line=0.2, las = 1)

# mtext(1, text=c("Abundance class"), adj=0.5, line=4.5, las = 1, outer=T)
# mtext(2, text=c("Native gamma richness"), adj=0.5, line=2.5, las = 0, outer=T)
...
plot(da, dg ,pch = 20, col=cgam , ann=F, type ="n", xlim =c(-4,0))
