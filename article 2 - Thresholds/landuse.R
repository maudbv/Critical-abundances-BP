...
tmp <- barplot(t(tab)[,2:dim(tab)[1]], col=LUCAS.col, main=sp, cex.axis=0.8, cex.names=0.8, las=1)
...
pasture.effect<- matrix(NA, nrow= length(sel), ncol = 10)
...
for (i in 1:length(sel))  {
  ...
  tbl <-  table(ab.vector$grasslands, ab.vector$abun)[,c(2,th+1)]
  (cor.pasture <- cor.test(prop.tab[2:dim(tab)[1],1],2:dim(tab)[1], method = "spearman", exact = FALSE))
  (t.pasture <- t.test(tbl[,1],tbl[,2], alternative = "two.sided"))
  (Ftest.pasture <-  fisher.test(tbl,simulate.p.value = TRUE,alternative = "two.sided"))
  ...
  tmp <- barplot(t(tab/rowSums(tab)), col=c("grey", "white"), main=sp,    cex.axis=0.8, cex.names=0.8, las=1)
  abline(v =tmp[M$impact.spread[sp,threshold]+1], lty="dashed")
...
## Native richness
M <- glmSRnat.overall
sel <- rownames(M$impact.spread) [which(!is.na(M$impact.spread$pth.CI))]
threshold = "pth.CI"

### test proportion of improved grassland at rare vs. threshold abundance
par(mfrow = c(2,3), mar=c(2,2,2,2), oma=c(3,3,2,3))
...
## ALIEN richness (impsp.ali)
M <- glmSRali.overall
sel <- rownames(M$impact.spread) [which(!is.na(M$impact.spread$pth.CI))]
threshold = "pth.CI"
### test proportion of improved grassland at rare vs. threshold abundance
par(mfrow = c(1,2), mar=c(2,2,2,2), oma=c(3,3,2,3))
