...
threshold = "th.CI"
sel <-c("AGRCAP","FESCOM" ,"HYPRAD", "VULBRO")
...
plot.glm(M = glmSRali.overall, var= "SRali",  db= db,sel.criteria = "sp.target", type= "overall.boot",
         ES = F, threshold = threshold, sp = sp.target, panel = c(1,4), ylim=c(-0.6,0.6))
...
threshold ="th.CI"
sp.target = c("AGRCAP","FESCOM","HYPRAD","VULBRO")
...
for (i in 1 : length(impsp)){
  ...
  # create y axis with the gamma richness Standardized effect size
  y =  (list.data$gamma.above[sp,] - list.data$null.above[sp,])
  y [ (1:6)[ n < 5 | is.na(n)] ]=NA
  y[1]=NA
  ...
  axis(1,at = 1:5, label = rep("",5), tcl= 0.1,mgp=c(1,0.5,0),las=1)
  if (i %in% c(8,9,10,11)) text(y=-54, x = 1:5, labels= abclasses[2:6],  cex=0.9, srt=45, adj=1, xpd = NA)
...
##### ALPHA mean above vs. below trends   ############


par(mfrow = c(3,4), mar=c(0,0,1,1), oma=c(6,6,2,1))

list.data <-alpha.above
alpha.loss <- as.data.frame(matrix(NA, nrow = length(impsp), ncol =6))
rownames(gamma.loss) <- impsp
...
for (i in 1 : length(impsp)){
  ...
  # create y axis with the gamma richness Standardized effect size
  y =  (list.data$alpha.above[i,] - list.data$null.above[i,])
  y [ (2:6)[ n < 5] ]=NA
  y[1]=NA
...
mtext(1, text=c("Abundance class"), adj=0.5, line=4, las = 1, outer=T)
mtext(2, text=c("loss in native alpha richness"), adj=0.5, line=3, las = 0, outer=T)
