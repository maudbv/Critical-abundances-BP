...
# initiating output
  ...
  gamma.list     <- list( obs= tmp , P.gamma = tmp, mean.null= tmp, sd.null = tmp, q975= tmp , q025 = tmp )
  rm(tmp)
...
# looping on each of the target species
  for (sp in spnames) {
    ...
    gamma.obs <- c(n <- sum(rowSums(community[var==1,])>0),
    ...
    for (k in 2: 6) {
      ...
      g <- ( if(sum(var==k)>1) {
        c(n <- sum(rowSums(community[var==k,])>0),
             ...
             shared <- sum(((colSums(community[var==k-1,])>0) + (colSums(community[var==k,])>0)) == 2),
             lost <- sum(((colSums(community[var==k-1,])>0) + (colSums(community[var==k,])==0)) == 2),
             gained <- sum(((colSums(community[var==k-1,])==0) + (colSums(community[var==k,])>0)) == 2)      
            )
      }
      else {
    ...
    gamma.obs<- rbind(gamma.obs, g)
    }
    gamma.obs<- as.data.frame(gamma.obs)
    rownames(gamma.obs) <- paste("c", 1: 6 )
   colnames(gamma.obs) <- c("nb.plot", "gamma", "alpha", "beta.add", "beta.mult","beta.prop",
                             "shared",  "lost","gained")
    ...
    for (j in 1:nreps) {
      ...
      r.gammas[j,] <- sapply(1:6, FUN = function(k){
        ...
        if (sum(rvar==k)>1) g <- sum(colSums(community[rvar==k,])>0)
        if (sum(rvar==k)==1) g <- sum(community[rvar==k,]>0)
        if (sum(rvar==k)==0) g <- 0
...
#           b <- (sum(colSums(community[rvar==k,])>0) - mean(rowSums(community[var==k,]), na.rm=T))/sum(colSums(community[rvar==k,])>0)
    ...
    r.betas <-  rbind( divpart.obs$beta.mult,  r.betas)
    
    beta.list$obs[sp,] <- divpart.obs$beta.mult
