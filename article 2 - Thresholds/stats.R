...
glm.table <- lapply(list(Total.richness = glmSR.overall, Native.richness = glmSRnat.overall, Alien.richness =glmSRali.overall), FUN= function (M) {
  tbl <- as.table(t(rbind(none = as.numeric(table(na=is.na(M$impact.spread$th), alien = rownames(M$impact.spread)%in%aliens)[2,]),
                          table(M$impact.spread$th, alien = rownames(M$impact.spread)%in%aliens))))
  return(tbl)
})
