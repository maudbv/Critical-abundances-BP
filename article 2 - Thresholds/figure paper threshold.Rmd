...
### Figure 2bis: trends in ALIEN alpha effect size for 11 species:
...
par(mfrow = c(2,2), mar=c(5,4,2,1), oma=c(0,0,0,0), las = 1)
...
plot(da, dg ,pch = 20, col=cgam , ann=F,ylim=c(-60,0), xlim= c(-5,0), type ="n")
...
# delta alpha vs spread
spread <- table.div.part$n.plot.impact
print(f <- cor.test(spread ,da))

plot (spread,da , pch = 20, col=cgam , ann=F, log = "x", xlim =c(2.5, 700), type= "n")
text(x = spread, y = da,label = rownames(table.div.part),col=cgam, cex =0.7)
...
list.data <-gamma.above.nat.total
...
--- ALIEN gamma richness
### trends in gamma ALIEN RICHNESS effect size for 12 species:
```{r gammatrendsSRali.impsp, echo=FALSE}
