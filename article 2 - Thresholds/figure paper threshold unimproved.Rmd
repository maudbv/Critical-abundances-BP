...
### Figure 3bis: trends in gamma observed for `r length(impsp)` species:
...
for (i in 1 : length(impsp)){
  ...
  sp <- impsp[i]
  # identify significant classes in black
  cols <- c(NA, "black") [ (gamma.trend.nat.total$P.gamma[sp,] <=0.025) +1]
  n <- as.numeric(glmSRnat.overall$n.obs[sp,])[1:6]
  # create x axis for the 5 abundance classes
  x = c(0:5)
  ...
  axis(1,at = 1:5, label = rep("",5), tcl= 0.1,mgp=c(1,0.5,0),las=1)
  if (i %in% c(10:13)) text(y=-57, x = 1:5, labels= abclasses[2:6],  cex=1, srt=45, adj=1, xpd = NA)
